...
const WHATSAPP = 'https://wa.me/60182085097'
...
export default function Contact({ nav }) {
  ...
  return (
    <>
      <nav style={{ position: 'sticky', top: 0, zIndex: 100, padding: '15px 0', background: 'rgba(4,4,10,0.85)', backdropFilter: 'blur(20px)', borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
      ...
      <div style={{ padding: '60px 0 90px' }}>
        <div style={{ ...W }}>
          <div className="mono" style={{ fontSize: '0.72rem', letterSpacing: '0.2em', color: '#F5C542', marginBottom: 14, textAlign: 'center' }}>✦ GET IN TOUCH</div>
          ...
          <p style={{ marginTop: 16, color: '#8A8AA0', fontSize: '0.98rem', textAlign: 'center', maxWidth: 480, margin: '16px auto 0', fontWeight: 300 }}>
            If a <span className="mono">yourname.harnova.my</span> link isn't enough, hit us up — we'll design and wire up a fully custom site on <span style={{ color: '#F4F4FA' }}>your own <span className="mono">.com</span></span> instead.
          </p>

          <div className="card" style={{ marginTop: 40, padding: 'clamp(24px,4vw,40px)' }}>
            {done ? (
              <div style={{ textAlign: 'center', padding: '20px 0' }}>
                <div style={{ width: 52, height: 52, borderRadius: '50%', background: 'rgba(57,255,20,0.1)', border: '1px solid rgba(57,255,20,0.35)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto' }}>
                  <Check size={24} color="#39FF14" />
                </div>
                <h2 style={{ fontSize: '1.15rem', fontWeight: 600, margin: '18px 0 8px' }}>Message sent.</h2>
                <p style={{ color: '#8A8AA0', fontSize: '0.92rem', maxWidth: 380, margin: '0 auto' }}>We'll get back to you at {email || 'your email'} — usually within a day. For anything urgent, WhatsApp us directly below.</p>
              </div>
            ) : (
              <form onSubmit={submit}>
                <div className="grid-2" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
                  <div>
                    <label style={{ display: 'block', fontSize: '0.85rem', color: '#B9B9CC', marginBottom: 8 }}>Name</label>
                    <input className="field" required value={name} maxLength={120} onChange={e => setName(e.target.value)} placeholder="Your name" />
                  </div>
                  <div>
                    <label style={{ display: 'block', fontSize: '0.85rem', color: '#B9B9CC', marginBottom: 8 }}>Email</label>
                ...
                <label style={{ display: 'block', fontSize: '0.85rem', color: '#B9B9CC', margin: '18px 0 8px' }}>What's this about?</label>
                ...
                <label style={{ display: 'block', fontSize: '0.85rem', color: '#B9B9CC', margin: '18px 0 8px' }}>Message</label>
          ...
          <div className="grid-2" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginTop: 20 }}>
            <a href={WHATSAPP} target="_blank" rel="noreferrer" className="card glass-btn" style={{ padding: 20, display: 'flex', alignItems: 'center', gap: 14 }}>
              <div style={{ width: 40, height: 40, borderRadius: 10, background: 'rgba(34,211,238,0.12)', border: '1px solid rgba(34,211,238,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                <MessageCircle size={18} color="#22D3EE" />
              </div>
              <div>
                <div style={{ fontWeight: 600, fontSize: '0.92rem' }}>WhatsApp us</div>
                <div style={{ fontSize: '0.8rem', color: '#8A8AA0' }}>Fastest way to reach us</div>
              </div>
              <ArrowUpRight size={15} color="#8A8AA0" style={{ marginLeft: 'auto', flexShrink: 0 }} />
            </a>
            <a href={`mailto:${CONTACT_EMAIL}`} className="card glass-btn" style={{ padding: 20, display: 'flex', alignItems: 'center', gap: 14 }}>
              <div style={{ width: 40, height: 40, borderRadius: 10, background: 'rgba(129,140,248,0.12)', border: '1px solid rgba(129,140,248,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                <Mail size={18} color="#818CF8" />
              </div>
              <div>
                <div style={{ fontWeight: 600, fontSize: '0.92rem' }}>Email us</div>
                <div className="mono" style={{ fontSize: '0.78rem', color: '#8A8AA0' }}>{CONTACT_EMAIL}</div>
              </div>
              <ArrowUpRight size={15} color="#8A8AA0" style={{ marginLeft: 'auto', flexShrink: 0 }} />
            </a>
          </div>

          <div className="card" style={{ marginTop: 16, padding: 20, display: 'flex', alignItems: 'center', gap: 14, background: 'linear-gradient(160deg, rgba(99,102,241,0.1), rgba(34,211,238,0.04))' }}>
            <Globe2 size={20} color="#F5C542" style={{ flexShrink: 0 }} />
            <p style={{ fontSize: '0.86rem', color: '#B9B9CC', lineHeight: 1.6 }}>
      ...
      <footer style={{ borderTop: '1px solid rgba(255,255,255,0.07)', padding: '30px 0' }}>
        <div style={{ ...W, maxWidth: 1000, display: 'flex', flexWrap: 'wrap', gap: 12, justifyContent: 'space-between', fontSize: '0.8rem', color: '#6E6E85' }}>
          ...
          <span style={{ display: 'inline-flex', gap: 18, alignItems: 'center' }}>
            ...
            <a href="/terms" onClick={e => { e.preventDefault(); nav('/terms') }} style={{ color: '#8A8AA0' }}>Terms</a>
          </span>
        </div>
      </footer>
    </>
