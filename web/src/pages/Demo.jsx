...
export default function Demo({ nav }) {
  ...
  return (
    <>
      ...
      <header style={{ padding: '60px 0 20px', textAlign: 'center' }}>
        <div style={{ ...W }}>
          <div className="mono" style={{ fontSize: '0.72rem', letterSpacing: '0.2em', color: '#F5C542', marginBottom: 14 }}>✦ TRY IT — NO SIGN-IN NEEDED</div>
          ...
          <p style={{ marginTop: 16, color: '#8A8AA0', fontSize: '0.98rem', maxWidth: 560, margin: '16px auto 0', fontWeight: 300 }}>
        ...
        <div style={{ ...W }}>
          <div className="card" style={{ padding: 'clamp(20px,3vw,30px)' }}>
            ...
            <div style={{ marginTop: 14, display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              {EXAMPLES.map(ex => (
                <button key={ex} onClick={() => { setPrompt(ex); generate(ex) }} disabled={busy} className="glass-btn"
                  style={{ padding: '7px 13px', borderRadius: 99, fontSize: '0.76rem', color: '#B9B9CC' }}>
                  {ex.length > 46 ? ex.slice(0, 46) + '…' : ex}
                </button>
              ))}
            </div>
            {remaining !== null && !err && (
              <div className="mono" style={{ marginTop: 12, fontSize: '0.72rem', color: '#8A8AA0' }}>{remaining} free {remaining === 1 ? 'try' : 'tries'} left today on this connection</div>
            )}
            ...
            {html && (
              <div style={{ marginTop: 22 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10, color: '#8A8AA0', fontSize: '0.82rem' }}>
                  <Eye size={13} /> Live preview
                </div>
                <iframe title="AI preview" sandbox="allow-scripts" srcDoc={html}
                  style={{ width: '100%', height: 460, borderRadius: 14, border: '1px solid rgba(255,255,255,0.13)', background: '#fff' }} />
      ...
      <section id="showcase" style={{ padding: '10px 0 90px', borderTop: '1px solid rgba(255,255,255,0.07)' }}>
        <div style={{ ...W, paddingTop: 50 }}>
          ...
          {showcase.length === 0 ? (
            <div className="card" style={{ marginTop: 30, padding: 40, textAlign: 'center', color: '#6E6E85' }}>
              ...
              {showcase.map(s => {
                ...
                return (
                  <a key={s.subdomain} href={url} target="_blank" rel="noreferrer" className="card" style={{ overflow: 'hidden', display: 'block' }}>
                    <div style={{ position: 'relative', height: 160, overflow: 'hidden', borderBottom: '1px solid rgba(255,255,255,0.08)', background: '#08080F' }}>
                    ...
                    <div style={{ padding: '14px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10 }}>
                      <div style={{ minWidth: 0 }}>
                        ...
                        <div className="mono" style={{ fontSize: '0.7rem', color: '#6E6E85', marginTop: 3 }}>{s.subdomain}.{root}</div>
                      </div>
                      <ArrowUpRight size={15} color="#8A8AA0" style={{ flexShrink: 0 }} />
                    </div>
                  </a>
                )
              })}
